...
while (true) {
opcion = await question("\n\nMasukkan metode sessions\n🔺 1 : scan qr\n🔺 2 : pair code\n\n\n")
if (opcion === '1' || opcion === '2') {
break
} else {
console.log("\n\n🔴 Masukkan hanya satu opsi \n\n 1 or 2\n\n" )
}}
opcion = opcion
}
...
if (opcion === '2' || methodCode) {
  ...
  if (MethodMobile) throw new Error('⚠️ Terjadi Kesalahan di API seluler')
  
  let addNumber
  if (!!phoneNumber) {
  addNumber = phoneNumber.replace(/[^0-9]/g, '')
  if (!Object.keys(PHONENUMBER_MCC).some(v => numeroTelefono.startsWith(v))) {
  console.log(chalk.bgBlack(chalk.bold.redBright("\n\nNomor Anda harus diawali dengan kode +62")))
  process.exit(0)
  }} else {
  while (true) {
  ...
  console.log(chalk.bgBlack(chalk.bold.redBright("\n\nPastikan untuk menambahkan kode negara")))
  }}
...
global.reloadHandler = async function (restatConn) {
  ...
  conn.sDesc = 'Deskripsi group telah di ubah menjadi\n@desc'
...
global.reload = async (_ev, filename) => {
  if (pluginFilter(filename)) {
    ...
    if (filename in global.plugins) {
      if (existsSync(dir)) conn.logger.info(`Plugin yang Diperbarui - '${filename}'`)
      else {
...
async function _quickTest() {
  ...
  if (!s.ffmpeg) conn.logger.warn('subscribe yt @ryuuotsuka')
  if (s.ffmpeg && !s.ffmpegWebp) conn.logger.warn('subscribe yt @ryuuotsuka')
  if (!s.convert && !s.magick && !s.gm) conn.logger.warn('subscribe yt @ryuuotsuka')
}

_quickTest()
  .then(() => conn.logger.info('✅ Tes cepat selesai!'))
  .catch(console.error)
