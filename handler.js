    ...
    try {
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (xp > 200)
                    m.reply('xp gak normal citer fix :v') // Hehehe
                else
                    m.exp += xp
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `Limit Anda terpakai habis\ngunakan perintah berikut untuk membeli lebih banyak limit\n\n*${usedPrefix}buy*`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `Level kamu kurang lebih dari cukup ${plugin.level} untuk menggunakan perintah ini. \n\nlevel kamu saat ini ${_user.level}`, m)
    ...
    switch (action) {
        ...
        case 'remove':
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        ...
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@group', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'Group ini tidak memiliki bio/desc') :
			    ...
			    mentionedJid: [user],
                            externalAdReply: {  
                            title: 'PihzBot MD',
                            body: action === 'add' ? 'wellcome' : 'good bye',
                            thumbnailUrl: pp,
                            sourceUrl: fake,
                            mediaType: 1,
            ...
            break
        case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user sekarang kamu adalah admin')
        case 'demote':
            let pp = await this.profilePictureUrl(participants[0], 'image').catch(_ => 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT9mFzSckd12spppS8gAJ2KB2ER-ccZd4pBbw&usqp=CAU') 
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '@user sekarang kamu bukan admin lagi')
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        unreg: `*ᴋʜᴜꜱᴜꜱ ᴜꜱᴇʀ ᴛᴇʀᴠᴇʀɪꜰɪᴋᴀꜱɪ •* ᴄᴏᴍᴍᴀɴᴅ ɪɴɪ ʜᴀɴʏᴀ ʙɪꜱᴀ ᴅɪ ᴘᴀᴋᴀɪ ᴋᴇᴛɪᴋᴀ ᴋᴀᴍᴜ ꜱᴜᴅᴀʜ ᴛᴇʀᴅᴀꜰᴛᴀʀ\n\nꜱɪʟᴀʜᴋᴀɴ ᴋᴇᴛɪᴋ .ᴅᴀꜰᴛᴀʀ ɴᴀᴍᴀ.ᴜᴍᴜʀ\n\nᴄᴏɴᴛᴏʜ: .ᴅᴀꜰᴛᴀʀ ʀʏᴜᴜ.22`,
...
watchFile(file, async () => {
    unwatchFile(file)
    console.log(chalk.magenta("✅  Ini telah diperbarui 'handler.js'"))
